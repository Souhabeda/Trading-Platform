...
export default function BuyCryptoSelect() {
    ...
    const onSubmit = (e) => {
        ...
        if (!pay || !receive) {
            toast.error('Failed request: Please fill in both "Pay" and "Receive" fields.');
            return;
        }
        router.push(`/buy-crypto-confirm?pay=${pay}&receive=${receive}&selectedPair=${selectedPair}&price=${price}`);
      };
    ...
    useEffect(() => {
        if (price && pay) {
            setReceive((parseFloat(pay) / price).toFixed(8));
        } else {
            setReceive('');
        }
    }, [pay, price]);
